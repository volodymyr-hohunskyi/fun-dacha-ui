...
class OCNP_Request {
   ...
   get url() {
      const token = 'user_token='+ (typeof getURLVar !== 'undefined' ? getURLVar('user_token') : new URLSearchParams(window.location.search).get('user_token'));
      const route = 'route=extension/ocnp/shipping/novaposhta.' + this._name;
...
async function syncItem(id){
   console.log('syncItem called with id:', id);
   ...
   if (!apiSettings.api_key || !apiSettings.api_url) {
      alert('Please enter API Key and API URL in the API Settings tab first!');
      return;
   }
   
   const novaposhta = new OCNP_NovaPoshta(apiSettings);
   
   // Find message container - try panel-body first, then card-body, then container-fluid
   let messageContainer = document.getElementsByClassName('panel-body')[0];
   if (!messageContainer) {
      messageContainer = document.getElementsByClassName('card-body')[0];
   }
   if (!messageContainer) {
      messageContainer = document.getElementsByClassName('container-fluid')[1]; // Second container-fluid is the content area
   }
   if (!messageContainer) {
      messageContainer = document.getElementById('content');
   }
   
   const uimessage = new OCNP_UserMessage(messageContainer || document.body);
   ...
   try {
      ...
      if (!response || !response.success) {
         ...
         uimessage.error(`fail to save settings: ${errorMsg}`);
         item.endSync();
         return;
      }
      ...
      if (response && response.success) {
         ...
         item.setCount(response.count || 0);
         uimessage.success(response.message || 'Sync completed successfully');
      }
      else {
         const errorMsg = response?.message || 'Unknown error occurred';
         uimessage.error(`synchronization failed: ${errorMsg}`);
      }
   }
   catch(err)
   {
