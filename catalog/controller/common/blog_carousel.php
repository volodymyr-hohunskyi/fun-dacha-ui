		...
		if ($results) {
			foreach ($results as $result) {
				if ($result['image']) {
					...
					$image = $this->model_tool_image->resize('placeholder.png', 400, 260);
				}
				...
				$data['articles'][] = [
					...
					'thumb'       => $image,
					'name'        => $result['name'],
					'description' => oc_substr(trim(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'))), 0, 120) . '...',
