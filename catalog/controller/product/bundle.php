...
class Bundle extends \Opencart\System\Engine\Controller {
	public function index(): \Opencart\System\Engine\Action|null {
		...
		$bundle_name = $bundle[$lang_key] ?? $bundle['name_uk'];

		$this->document->setTitle($bundle_name . ' | ' . $this->config->get('config_name'));
		$this->document->setDescription($bundle['description_uk'] ?? '');
		...
		$data['breadcrumbs'][] = [
			'text' => $bundle_name,
			'href' => $this->url->link('product/bundle', 'language=' . $this->config->get('config_language') . '&slug=' . $slug)
		];

		$data['heading_title'] = $bundle_name;
		$data['description'] = $bundle['description_uk'] ?? '';
		$data['icon'] = $bundle['icon'] ?? 'fa-box';
		...
		if (!empty($bundle['product_ids'])) {
			foreach ($bundle['product_ids'] as $pid) {
			...
			foreach ($bundle['category_ids'] ?? [] as $cat_id) {
		...
		$this->response->setOutput($this->load->view('product/bundle', $data));
		return null;
	}
}
