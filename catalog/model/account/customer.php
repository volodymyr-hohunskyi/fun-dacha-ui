...
class Customer extends \Opencart\System\Engine\Model {
	...
	public function addCustomer(array $data): int {
		...
		// Ensure password is set and not empty
		$password = isset($data['password']) ? html_entity_decode($data['password'], ENT_QUOTES, 'UTF-8') : '';
		if (empty($password)) {
			// If password is empty, generate a random one (should not happen if validation works)
			$password = bin2hex(random_bytes(16));
		}
		
		$password_hash = password_hash($password, PASSWORD_DEFAULT);

		$this->db->query("INSERT INTO `" . DB_PREFIX . "customer` SET `customer_group_id` = '" . (int)$customer_group_id . "', `store_id` = '" . (int)$this->config->get('config_store_id') . "', `language_id` = '" . (int)$this->config->get('config_language_id') . "', `firstname` = '" . $this->db->escape($data['firstname']) . "', `lastname` = '" . $this->db->escape($data['lastname']) . "', `email` = '" . $this->db->escape(oc_strtolower($data['email'])) . "', `telephone` = '" . $this->db->escape($data['telephone']) . "', `custom_field` = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : '') . "', `password` = '" . $this->db->escape($password_hash) . "', `newsletter` = '" . (isset($data['newsletter']) ? (int)$data['newsletter'] : 0) . "', `ip` = '" . $this->db->escape(oc_get_ip()) . "', `status` = '" . (int)!$customer_group_info['approval'] . "', `date_added` = NOW()");
