...
- Product `tags` are converted into filters automatically using `shared/data/tags.csv`. The builder creates `FilterGroups`, `Filters`, `CategoryFilters`, and `ProductFilters` worksheets with Ukrainian/Russian labels so the stock filter module can list the right options per category.
